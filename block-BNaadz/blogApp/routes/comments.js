...
router.get('/:id/edit', (req, res, next) => {
    ...
    Comment.findById(id).populate('author', 'fullName').exec((err, comment) => {
        if(comment.author.id !== req.user.id) {
            return res.redirect('/articles/' + commennt.articleId);
        }
...
router.post('/:id', (req, res, next) => {
    var id = req.params.id;
    Comment.findById(id).populate('author', 'fulllName').exec((err, comment) => {
